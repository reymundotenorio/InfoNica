<?php 

function titulo_foto($Nombre_Foto,$Seccion){

    include_once("conectar.php");
    $conexion = new Conexion();
    $conexion->ConectarMySql();
    $con = $conexion->Conectar;

$resultado = mysqli_query($con,"SELECT ID_Seccion FROM Seccion WHERE (Nombre_Seccion = '$Seccion')");
$rows = mysqli_fetch_array($resultado,MYSQLI_BOTH);

$ID_Seccion = $rows["ID_Seccion"];

$result = mysqli_query($con,"SELECT Titulo_Foto FROM Foto WHERE ((Nombre_Foto = '$Nombre_Foto') and (ID_Seccion = '$ID_Seccion'))");

$row = mysqli_fetch_array($result,MYSQLI_BOTH);

$Titulo_Foto = $row["Titulo_Foto"];

if(!empty($Titulo_Foto)){
    echo "<h4>".$Titulo_Foto."</h4>\n";
}
else{
    echo "<h4>".$Nombre_Foto."</h4>\n"; 
}

mysqli_close($con);
}

 ?>